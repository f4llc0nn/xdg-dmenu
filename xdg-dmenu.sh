...
apps_path="/usr/share/kali-menu/applications/"
...
[[ "${submenu_selected:0:${#menu_prefix}}" == "$menu_prefix" ]] && {
    ...
    [[ ! "$toolexec" ]] && exit 1 || find "$apps_path" -iname "*$toolexec*.desktop" -exec grep "Exec" {} \; |cut -d"=" -f2 |source /dev/stdin
} || {
    toolexec="$submenu_selected"
    find "$apps_path" -iname "*$toolexec*.desktop" -exec grep "Exec" {} \; |cut -d"=" -f2 |source /dev/stdin
}
